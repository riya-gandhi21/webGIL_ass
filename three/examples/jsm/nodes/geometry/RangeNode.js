...
class RangeNode extends Node {
  ...
  getNodeType(builder) {
    return builder.object.isInstancedMesh === true
  ...
  construct(builder) {
    ...
    if (object.isInstancedMesh === true) {
      ...
      if (
        attributeGeometry === undefined ||
        attributeGeometry.array.length < length
      ) {
        if (vectorLength === 1) {
        ...
        geometry.setAttribute(
          attributeName,
          new InstancedBufferAttribute(array, vectorLength)
        );
      ...
      output = attribute(
        attributeName,
        builder.getTypeFromLength(vectorLength)
      );
    } else {
      output = float(0);
    }
