...
export default class MeshStandardNodeMaterial extends NodeMaterial {
  ...
  build(builder) {
    ...
    if (materialLightsNode.length > 0) {
      builder.lightsNode = new LightsNode([
        ...builder.lightsNode.lightNodes,
        ...materialLightsNode,
      ]);
    }
  ...
  generateStandardMaterial(builder, { colorNode, diffuseColorNode }) {
    ...
    let metalnessNode = this.metalnessNode
      ? float(this.metalnessNode)
      : materialMetalness;

    metalnessNode = builder.addFlow(
      "fragment",
      label(metalnessNode, "Metalness")
    );
    builder.addFlow(
      "fragment",
      assign(
        diffuseColorNode,
        vec4(
          mul(diffuseColorNode.rgb, invert(metalnessNode)),
          diffuseColorNode.a
        )
      )
    );
  ...
  generateLight(
    builder,
    { diffuseColorNode, lightingModelNode, lightsNode = builder.lightsNode }
  ) {
    ...
    outgoingLightNode = add(
      vec3(this.emissiveNode || materialEmissive),
      outgoingLightNode
    );
    ...
    if (renderer.toneMappingNode)
      outgoingLightNode = context(renderer.toneMappingNode, {
