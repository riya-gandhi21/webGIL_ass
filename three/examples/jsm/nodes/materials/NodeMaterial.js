...
class NodeMaterial extends ShaderMaterial {
  ...
  generateDiffuseColor(builder) {
    ...
    let opacityNode = this.opacityNode
    ...
    const diffuseColorNode = builder.addFlow(
      "fragment",
      label(colorNode, "DiffuseColor")
    );
    ...
    builder.addFlow(
      "fragment",
      assign(diffuseColorNode.a, mul(diffuseColorNode.a, opacityNode))
    );

    // ALPHA TEST

    if (this.alphaTestNode || this.alphaTest > 0) {
      const alphaTestNode = this.alphaTestNode
      ...
      builder.addFlow(
        "fragment",
        new ExpressionNode("if ( DiffuseColor.a <= AlphaTest ) { discard; }")
      );
    }
  ...
  generateLight(
    ...
    if (lightsNode && lightsNode.hasLight !== false)
      outgoingLightNode = builder.addFlow(
        "fragment",
