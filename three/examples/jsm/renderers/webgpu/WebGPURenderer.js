...
Matrix4.prototype.makeOrthographic = function (
  left,
  right,
  top,
  bottom,
...
class WebGPURenderer {
  constructor(parameters = {}) {
    ...
    if (this._parameters.antialias === true) {
      this._parameters.sampleCount =
        parameters.sampleCount === undefined ? 4 : parameters.sampleCount;
    } else {
  ...
  async init() {
    ...
    const context =
      parameters.context !== undefined
        ? parameters.context
        : this.domElement.getContext("webgpu");
    ...
    this._bindings = this._renderPipelines.bindings = new WebGPUBindings(
      device,
      this._info,
      this._properties,
      this._textures,
  ...
  render(scene, camera) {
    ...
    if (renderTarget !== null) {
      ...
      colorAttachment.view =
        renderTargetProperties.colorTextureGPU.createView();
      depthStencilAttachment.view =
        renderTargetProperties.depthTextureGPU.createView();
    } else {
      if (this._parameters.antialias === true) {
        colorAttachment.view = this._colorBuffer.createView();
        colorAttachment.resolveTarget = this._context
          .getCurrentTexture()
          .createView();
      } else {
  ...
  _renderObject(
    ...
    if (hasIndex === true) {
      const indexCount =
        drawRange.count !== Infinity ? drawRange.count : index.count;
