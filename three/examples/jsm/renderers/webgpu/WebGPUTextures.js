...
class WebGPUTextures {
    ...
    if (renderTargetProperties.initialized === undefined) {
      ...
      if (renderTarget.depthBuffer === true) {
        ...
        if (renderTarget.depthTexture !== null) {
          const depthTextureProperties = properties.get(
  ...
  _uploadTexture(texture) {
    ...
    const mipLevelCount = this._getMipLevelCount(
      texture,
      width,
      height,
      needsMipmaps
  ...
  _copyCubeMapToTexture(
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (image.isDataTexture) {
        ...
        if (needsMipmaps === true)
          this._generateMipmaps(textureGPU, textureGPUDescriptor, i);
      } else {
        this._getImageBitmap(image, texture).then((imageBitmap) => {
...
function onRenderTargetDispose(event) {
  ...
  renderTarget.removeEventListener(
    "dispose",
    renderTargetProperties.disposeCallback
  );
