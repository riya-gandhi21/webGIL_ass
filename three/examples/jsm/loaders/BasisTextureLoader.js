      ...
      this.transcoderPending = Promise.all([jsContent, binaryContent]).then(
        ([jsContent, binaryContent]) => {
          ...
          const body = [
            "/* constants */",
            "let _EngineFormat = " +
              JSON.stringify(BasisTextureLoader.EngineFormat),
            "let _TranscoderFormat = " +
              JSON.stringify(BasisTextureLoader.TranscoderFormat),
            "let _BasisFormat = " +
              JSON.stringify(BasisTextureLoader.BasisFormat),
            "/* basis_transcoder.js */",
            jsContent,
            "/* worker */",
...
BasisTextureLoader.BasisWorker = function () {
    ...
    switch (message.type) {
      ...
      case "transcode":
        transcoderPending.then(() => {
          try {
            const { width, height, hasAlpha, mipmaps, format } = message
              .taskConfig.lowLevel
  ...
  function transcodeLowLevel(taskConfig) {
    ...
    const blockByteLength =
      BasisModule.getBytesPerBlockOrPixel(transcoderFormat);
