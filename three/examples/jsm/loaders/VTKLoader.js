      ...
      function parseDataArray(ele, compressed) {
        ...
        if (ele.attributes.format === "binary" && compressed) {
          if (ele.attributes.type === "Float32") {
            ...
            ele.attributes.type === "Int32" ||
            ele.attributes.type === "Int64"
          ) {
            txt = new Int32Array();
          }
        ...
        while (sectionIndex < numberOfSections) {
          ...
          if (section && section.DataArray) {
            ...
            switch (sections[sectionIndex]) {
              ...
              case "Strips":
                {
                  const numberOfStrips = parseInt(
                    piece.attributes.NumberOfStrips
                  );

                  if (numberOfStrips > 0) {
                    const connectivity = new Int32Array(
                      section.DataArray[0].text.length
                    );
                    const offset = new Int32Array(
