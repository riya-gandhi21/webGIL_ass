...
class ThreeMFLoader extends Loader {
  ...
  parse(data) {
    ...
    function loadDocument(data) {
      ...
      try {
        ...
        if (e instanceof ReferenceError) {
          console.error(
            "THREE.3MFLoader: fflate missing and file is compressed."
          );
          return null;
        }
      }
    ...
    function parseRelsXml(relsFileText) {
      ...
      const relsXmlData = new DOMParser().parseFromString(
        relsFileText,
        "application/xml"
      );
    ...
    function parseResourcesNode(resourcesNode) {
      ...
      const pbmetallicdisplaypropertiesNodes = resourcesNode.querySelectorAll(
        "pbmetallicdisplayproperties"
      );

      for (let i = 0; i < pbmetallicdisplaypropertiesNodes.length; i++) {
        const pbmetallicdisplaypropertiesNode =
          pbmetallicdisplaypropertiesNodes[i];
        const pbmetallicdisplaypropertiesData =
    ...
    function buildBasematerial(materialData, objects, modelData) {
      ...
      const pbmetallicdisplayproperties =
      ...
      if (
        ...
        const pbmetallicdisplayproperty =
