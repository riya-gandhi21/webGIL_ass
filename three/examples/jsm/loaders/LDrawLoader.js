    ...
    while (locationState !== FILE_LOCATION_NOT_FOUND) {
      ...
      switch (locationState) {
        ...
        case FILE_LOCATION_TRY_RELATIVE:
          subobjectURL =
            fileName.substring(0, fileName.lastIndexOf("/") + 1) + subobjectURL;
          locationState = locationState + 1;
          break;
...
function getMaterialFromCode(
  ...
  const isPassthrough =
    (!forEdge && colorCode === MAIN_COLOUR_CODE) ||
    (forEdge && colorCode === MAIN_EDGE_COLOUR_CODE);
  if (isPassthrough) {
    ...
    const processInfoSubobjects = async (info, subobject = null) => {
      ...
      for (let i = 0, l = subobjects.length; i < l; i++) {
        ...
        const promise = parseCache
          .ensureDataLoaded(subobject.fileName)
          .then(() => {
      ...
      for (let i = 0, l = subobjectInfos.length; i < l; i++) {
        ...
        if (subobjectInfo.isGroup) {
          ...
          subobject.matrix.decompose(
            subobjectGroup.position,
            subobjectGroup.quaternion,
            subobjectGroup.scale
          );
        ...
        const lineColorCode =
          colorCode === MAIN_COLOUR_CODE ? MAIN_EDGE_COLOUR_CODE : colorCode;
        for (let i = 0, l = lineSegments.length; i < l; i++) {
          const ls = lineSegments[i];
          const vertices = ls.vertices;
          ...
          ls.colorCode =
            ls.colorCode === MAIN_EDGE_COLOUR_CODE
              ? lineColorCode
              : ls.colorCode;
          ls.material =
            ls.material ||
        ...
        for (let i = 0, l = conditionalSegments.length; i < l; i++) {
          ...
          os.colorCode =
            os.colorCode === MAIN_EDGE_COLOUR_CODE
              ? lineColorCode
              : os.colorCode;
          os.material =
            os.material ||
        ...
        for (let i = 0, l = faces.length; i < l; i++) {
          ...
          tri.colorCode =
            tri.colorCode === MAIN_COLOUR_CODE ? colorCode : tri.colorCode;
          tri.material =
            tri.material ||
            getMaterialFromCode(
              tri.colorCode,
...
function createObject(
  ...
  const normals =
    elementSize === 3
      ? new Float32Array(elementSize * totalElements * 3)
      : null;
  const materials = [];
  ...
  if (elementSize === 2) {
    if (isConditionalSegments) {
      object3d = new ConditionalLineSegments(
        bufferGeometry,
        materials.length === 1 ? materials[0] : materials
      );
    } else {
      object3d = new LineSegments(
        bufferGeometry,
        materials.length === 1 ? materials[0] : materials
      );
    }
...
class LDrawLoader extends Loader {
  constructor(manager) {
    ...
    this.missingConditionalEdgeColorMaterial = new LDrawConditionalLineMaterial(
      { fog: true, color: 0xff00ff }
    );
    this.missingConditionalEdgeColorMaterial.name =
      "Missing material - Conditional Edge";
    this.missingColorMaterial.userData.edgeMaterial =
      this.missingEdgeColorMaterial;
    this.missingEdgeColorMaterial.userData.conditionalEdgeMaterial =
  ...
  async preloadMaterials(url) {
    ...
    for (let i = 0, l = lines.length; i < l; i++) {
      ...
      if (colorLineRegex.test(line)) {
        ...
        const material = this.parseColorMetaDirective(
          new LineParser(directive)
        );
        materials.push(material);
      }
    }
  ...
  applyMaterialsToMesh(
    ...
    function getMaterial(c, colorCode) {
      ...
      if (
        parentIsPassthrough &&
        !(colorCode in materialHierarchy) &&
        !finalMaterialPass
      ) {
        return colorCode;
      }
      ...
      if (colorCode in materialHierarchy) {
        ...
        if (material === null) {
          ...
          console.warn(
            `LDrawLoader: Material properties for code ${colorCode} not available.`
          );
  ...
  parseColorMetaDirective(lineParser) {
        ...
        switch (token.toUpperCase()) {
          ...
          case "EDGE":
            ...
            if (edgeColor.startsWith("0x")) {
              ...
              if (!edgeMaterial) {
                throw new Error(
                  "LDrawLoader: Invalid edge color while parsing material" +
                    lineParser.getLineNumberString() +
                    "."
                );
              }
    ...
    if (!edgeMaterial) {
      ...
      edgeMaterial.userData.conditionalEdgeMaterial.name =
