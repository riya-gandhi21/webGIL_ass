...
function supported(renderer, renderTarget) {
  if (!renderer || !renderer.isWebGLRenderer) {
    console.error(
      "EXRExporter.parse: Unsupported first parameter, expected instance of WebGLRenderer."
    );
  ...
  if (!renderTarget || !renderTarget.isWebGLRenderTarget) {
    console.error(
      "EXRExporter.parse: Unsupported second parameter, expected instance of WebGLRenderTarget."
    );
  ...
  if (
    ...
    console.error(
      "EXRExporter.parse: Unsupported WebGLRenderTarget texture type."
    );
  ...
  if (renderTarget.texture.format !== RGBAFormat) {
    console.error(
      "EXRExporter.parse: Unsupported WebGLRenderTarget texture format, expected RGBAFormat."
    );
...
function buildInfo(renderTarget, options = {}) {
  ...
  const WIDTH = renderTarget.width,
    ...
    COMPRESSION =
      options.compression !== undefined ? options.compression : ZIP_COMPRESSION,
...
function getPixelData(renderer, rtt, info) {
  ...
  if (info.type === FloatType) {
    dataBuffer = new Float32Array(
      info.width * info.height * info.numInputChannels
    );
  } else {
    dataBuffer = new Uint16Array(
      info.width * info.height * info.numInputChannels
    );
  }
...
function reorganizeDataBuffer(inBuffer, info) {
  const w = info.width,
    ...
    outBuffer = new Uint8Array(
      info.width * info.height * info.numOutputChannels * info.dataSize
    ),
  ...
  for (let y = 0; y < h; ++y) {
    for (let x = 0; x < w; ++x) {
      ...
      offset.value =
        line + (1 + cOffset) * w * info.dataSize + x * info.dataSize;
      setValue(dv, dec.g, offset);

      offset.value =
        line + (2 + cOffset) * w * info.dataSize + x * info.dataSize;
...
function compressData(inBuffer, info) {
  ...
  const chunks = { data: new Array(), totalSize: 0 },
    size =
      info.width * info.numOutputChannels * info.blockLines * info.dataSize;
...
function fillData(chunks, info) {
  const TableSize = info.numBlocks * 8,
    ...
    outBuffer = new Uint8Array(
      HeaderSize + TableSize + chunks.totalSize + info.numBlocks * 8
    ),
    dv = new DataView(outBuffer.buffer);
